...
class SPDU:
    ...
    def pack(self):
        return  self.SPDU_CPC+ self.SPDU_length_1 + self.SPDU_PDV + self.SPDU_length_2 + self.SPDU_context_1 + self.SPDU_context_2 + self.SPDU_ASN + self.SPDU_length_3
...
def get_fuzzing_bytes():
    print('Please input byte number:')
...
def fuzz(ip,byte_number):
    ...
    for count1 in range(byte_number,len(mutation_bytes)) :
...
def main():
    args = get_args()
    byte_number = get_fuzzing_bytes()
    fuzz(args.ip,byte_number)
    print("fuzzer finished===========")
